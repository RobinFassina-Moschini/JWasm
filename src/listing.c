...
#define CODELINE2 1 /* v2.18: emit a second line for code lines if first line cannot render all bytes */
...
/*************************************************************/
{
    ...
    if ( ( Parse_Pass > PASS_1 ) && UseSavedState ) {
        if ( ModuleInfo.GeneratedCode == 0 ) {
            ...
            if ( ModuleInfo.CurrComment ) { /* if comment was removed, read it! */
    ...
    case LSTTYPE_CODE:
...
#if CODELINE2
			/* v2.18: add a second line if codebytes won't fit in the first */
			if ( type == LSTTYPE_CODE && newofs - oldofs > len ) {
				ll.next = myalloca( sizeof( struct lstleft ) );
				ll.next->next = NULL;
				memset( ll.next->buffer, ' ', sizeof( ll.next->buffer) );
			}
#endif
            break; /* todo: check code and see if a second line may be needed. */
        }
         ...
         * todo: don't get the contents from CodeBuffer. For code,
		...
		/* v2.18: mark relocations in listing bytes.
		 * todo: allow a second/third line.
		 */
		if ( type == LSTTYPE_CODE && value ) {
			...
			DebugMsg1(("LstWrite: opnd[0].type=0x%" I32_SPEC "X fixup=%p\n", CodeInfo->opnd[0].type, CodeInfo->opnd[0].InsFixup));
			while ( oldofs < newofs ) {
				if ( CodeInfo->opnd[oidx].type != OP_NONE &&
					...
					char *pv = &CurrSeg->e.seginfo->CodeBuffer[idx];
					...
					case 4:
						if ( CodeInfo->opnd[oidx].InsFixup->type == FIX_PTR16 )
							...
							p2 += sprintf( p2, " %08X%c", *(uint_32 *)pv, cType );
						break;
					case 6: p2 += sprintf( p2, " %08Xo %04Xs", *(uint_32 *)pv, *(uint_16 *)(pv+4) ); break;
#if AMD64_SUPPORT
					case 8: p2 += sprintf( p2, " %16X%c", *(uint_64 *)pv, cType ); break;
#endif
					}
					idx += cSize;
					oldofs += cSize;
    ...
    *p2 = ' ';
    ...
    if ( Parse_Pass == PASS_1 || UseSavedState == FALSE ) {
        ...
        DebugMsg1(("LstWrite: writing (%u b) >%s< len=%u, len2=%u\n", idx, ll.buffer, len, len2 ));
#if CODELINE2
		/* problem: len2 usually is 0, because the comment isn't stored in line store.
		 * To get the full line size the "next" member has to be queried.
		 * Works with one additional line only!
		 */
		if ( ll.next && LineStoreCurr->next )
			list_pos = LineStoreCurr->next->list_pos - ( sizeof( ll.buffer ) + 2 );
		else
#endif
            list_pos += sizeof( ll.buffer ) + len + len2 + NLSIZ;
#if CODELINE2
		if ( ll.next ) /* v2.18: currently just 1 additional line possible in pass > 1 */
			fseek( CurrFile[LST], list_pos, SEEK_SET );
#endif
#ifdef DEBUG_OUT
        cntLstBytes += idx;
